...
const TASKS: Omit<Task, "status">[] = [
  ...
  },
  {
    ...
    description: "Add the FarFISH mini app to your account",
    reward: 40,
    type: "miniapp",
  },
  {
...
function VerifyModal({ 
  ...
  const getTaskText = () => {
    switch (taskId) {
      case "fc_follow":
        return "Follow @farf on the platform";
      case "fc_like_recast":
...
export default function SteamPage() {
  ...
  const handleVerify = (taskId: string, taskTitle: string) => {
    ...
    if (task?.type === "daily") {
      ...
      console.log("[STEAM] Miniapp verify clicked:", { taskId, walletAddress: wallet });
      setVerifyModal({
        isOpen: true,
        taskId,
        taskTitle,
  ...
  const handleDailyCheckin = async () => {
    ...
    try {
      // Call API to handle the fishing check-in with Upstash KV storage
      const response = await fetch('/api/steam/fishing', {
        method: 'POST',
        headers: {
                  ...
                  {task.status === "verified" ? (
                    ...
                    <>
                      ...
                      {(task.type === "farcaster" || task.type === "daily") && (
                        ...
                        >
                          {task.type === "daily" ? "Check In" : "Verify via Social"}
                        </button>
                      )}
                      {task.type === "miniapp" && (
                        <>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
          ...
          <div className="space-y-2 text-white/80">
            <div className="flex items-start gap-2">
              <span>•</span>
              <span>Complete tasks on Steam page everyday.</span>
            </div>
            <div className="flex items-start gap-2">
              <span>•</span>
              <span>Rewards are accumulated on Steam</span>
            </div>
            <div className="flex items-start gap-2">
              <span>•</span>
              <span>No instant claiming</span>
            </div>
            <div className="flex items-start gap-2">
              <span>•</span>
              <span>Rewards are distributed later before listing</span>
            </div>
          </div>
        </div>
      </div>
