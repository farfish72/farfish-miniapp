...
import { NFT_CONTRACT_ADDRESS, CLAIM_CONTROLLER_ADDRESS } from "../constants";
...
type Task = {
  ...
  description: string;
  reward: number;
  type: "daily" | "farcaster" | "referral" | "referral_milestone" | "nft";
  status: TaskStatus;
...
const TASKS: Omit<Task, "status">[] = [
  ...
  },
  {
    ...
    description: "Follow the official FarFISH account on Farcaster",
    reward: 50,
    type: "farcaster",
  },
  {
    ...
    description: "Like and recast the FarFISH announcement cast",
    reward: 25,
    type: "farcaster",
  },
  {
...
export default function SteamPage() {
  ...
  // Daily check-in data (same logic as Chest page)
  ...
  const fetchTaskStatuses = useCallback(async () => {
    try {
      ...
      const withStatus = TASKS.map((task) => {
        ...
        if (task.type === "daily") {
          // Daily check-in uses same logic as Chest page - can claim means not completed today
          status = dailyData && dailyData[0] ? "not_started" : "verified";
        } else if (task.type === "farcaster") {
          // Farcaster tasks use stored verification
  ...
  }, [wallet, dailyData, nftBalance, activeStakes, referralData.count]);
  ...
  const handleVerify = (taskId: string, taskTitle: string) => {
    ...
    if (task?.type === "daily") {
      ...
      console.log("[STEAM] Verify clicked:", { taskId, walletAddress: wallet, fid: "will_fetch_from_sdk" });
      setVerifyModal({
        isOpen: true,
        taskId,
        taskTitle,
  ...
  const handleDailyCheckin = async () => {
    if (!wallet || !dailyData || !dailyData[0]) return;

    try {
      // Same logic as Chest page - update localStorage
    ...
    <div className="flex flex-col flex-1 min-h-screen pb-28">
      ...
      <div className="flex-1 space-y-6 mt-4">
                  ...
                  {task.status === "verified" ? (
                    ...
                    <>
                      ...
                      {(task.type === "farcaster" || task.type === "daily") && (
                        ...
                        >
                          {task.type === "daily" ? "Check In" : "Verify via Farcaster"}
                        </button>
                      )}
                      {task.type === "referral" && (
                        <div className="text-xs text-white/60 text-center">
                          Share your referral link<br />to earn rewards
                        </div>
                      )}
                      {task.type === "referral_milestone" && (
                        <div className="text-xs text-white/60 text-center">
        ...
        <div className="text-center text-white/60 text-sm py-4">
          Verification is performed inside Farcaster.
          <br />
          This page only displays task status.
        </div>
      </div>
